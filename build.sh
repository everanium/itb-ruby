...
if [[ "${ITB_SKIP_CLEAN:-0}" == "1" ]]; then
    ...
    if [[ "${ITB_KEEP_DOWNLOADS:-0}" == "1" ]]; then
        echo "[clean] ITB_KEEP_DOWNLOADS=1: keeping vendor/, .bundle/ and Gemfile.lock (weaker guarantee)"
    else
        clean_target 'vendor'
        clean_target '.bundle'
        clean_target 'Gemfile.lock'
    fi
fi
...
echo "==> Ruby binding loads libitb3.so at runtime via the ffi gem; no further build step."
