...
(`~> 1.17`). Windows: the same; libitb3 builds as `libitb3.dll`. The
`ffi` gem is portable across MRI, JRuby, and TruffleRuby.

## Build the shared library

The convenience driver builds `libitb3.so` and syntax-checks the Ruby
sources in one step:
...
1. `ITB_LIBITB3_PATH` environment variable (path to the shared
   library file).
2. `<repo>/dist/<os>-<arch>/libitb3.<ext>` resolved from the gem
...
libitb3 emits; absent keys are optional fields at their zero value. The
method is named `inspect_blob` rather than `inspect` because
`Module#inspect` is Ruby's own debug-representation hook.
...
- libitb3 must be reachable at load time through the lookup order
  above; a resolve failure raises `LoadError` at `require "libitb3"`.
