...
require "libitb3/version"
require "libitb3/ffi_bridge"
require "libitb3/pipeline"
require "libitb3/stream"
require "libitb3/errors"

# Thin Ruby proxy over the libitb3 shared library's Triple Pipeline
# surface.
#
# The gem wraps the ITB_Triple_* C ABI exported by cmd/cshared
# (libitb3.so / .dylib / .dll) through the ffi gem -- runtime FFI, no
...
module ITB
  ...
  class << self
    ...
    # Pipeline and returns it as a Hash over the JSON object libitb3
    # emits. No registry read, no primitive probe -- a primitive name
    ...
    # reserved prefixes, field rules) is performed by libitb3; a
    ...
    # buffer, then a standard-library JSON decode of the bytes libitb3
    # wrote.
    def json_out(&call)
