...
module ITB
  # Runtime symbol loading over the libitb3 shared library (ffi gem).
  #
  ...
  module FFIBridge
    ...
    def self.lib_filename
      ...
      when /mswin|mingw|cygwin/ then "libitb3.dll"
      when /darwin/ then "libitb3.dylib"
      else "libitb3.so"
      end
    end
    ...
    begin
      ...
      raise LoadError,
            "itb: failed to load libitb3 (tried #{library_candidates.join(', ')}): #{e.message}"
    end
